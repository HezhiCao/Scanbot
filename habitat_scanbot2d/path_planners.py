...
class DifferentiableStarPlanner(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.neights2channels.weight.data = torch.from_numpy(
            self._init_neights_to_channels(3)
        )
        self.neights2channels.to(device)
        if preprocessing:
            self._init_preprocessing_kernel()
    ...
    def forward(
        ...
        if self.preprocessing:
            self.obstacles = self.preprocessing_kernel(obstacles.to(self.device))
        else:
        ...
        if self.visualize:
            ...
            self.g_map_image = self.axes[0].imshow(
                self.g_map.squeeze().cpu().detach().numpy().astype(np.float32),
                vmin=0.0,
                vmax=self.ob_cost * 1.2,
                animated=True,
            )
            self.close_list_image = self.axes[1].imshow(
                self.close_list_map.squeeze().cpu().detach().numpy().astype(np.float32),
                vmin=0.0,
                vmax=1.0,
                animated=True,
            )
            self.open_list_image = self.axes[2].imshow(
                self.open_list_map.squeeze().cpu().detach().numpy().astype(np.float32),
                vmin=0.0,
                vmax=1.0,
                animated=True,
            )
            self.axes[0].set_title("g_map")
            self.axes[1].set_title("close_list_map")
            self.axes[2].set_title("open_list_map")
            divider = make_axes_locatable(self.axes[0])
            cax = divider.append_axes("right", "5%", pad="3%")
            self.fig.colorbar(self.g_map_image, cax=cax)
            divider = make_axes_locatable(self.axes[1])
            cax = divider.append_axes("right", "5%", pad="3%")
            self.fig.colorbar(self.close_list_image, cax=cax)
            divider = make_axes_locatable(self.axes[2])
            cax = divider.append_axes("right", "5%", pad="3%")
            self.fig.colorbar(self.open_list_image, cax=cax)
        ...
        while not_done:
            ...
            if self.visualize:
                self.g_map_image.set_data(
                    self.g_map.squeeze().cpu().detach().numpy().astype(np.float32)
                )
                self.close_list_image.set_data(
                    self.close_list_map.squeeze()
                    .cpu()
                    .detach()
                    .numpy()
                    .astype(np.float32)
                )
                self.open_list_image.set_data(
                    self.open_list_map.squeeze()
                    .cpu()
                    .detach()
                    .numpy()
                    .astype(np.float32)
            ...
            if step >= self.max_steps:
                ...
                break
            not_done = (self.close_list_map.view(-1)[goal_idx].item() < 0.1) or (
        ...
        out = torch.cat(
            [
                ...
                * torch.max(
                ...
                torch.sqrt(left_diff_sq + self.eps) + self.ob_cost
                # * torch.max(
                #     obstacles_pd[:, :, 0:h, 1 : w + 1],
                #     obstacles_pd[:, :, 1 : h + 1, 1 : w + 1],
                # ),
                * torch.max(
                    obstacles_pd[:, :, 1 : h + 1, 0:w],
                    obstacles_pd[:, :, 1 : h + 1, 1 : w + 1],
                ),
    ...
    @staticmethod
    def _init_neights_to_channels(ks: int = 3):
    ...
    def _init_preprocessing_kernel(self):
        self.preprocessing_kernel = nn.Conv2d(
            1, 1, kernel_size=(3, 3), padding=1, bias=False, device=self.device
        )
        self.preprocessing_kernel.weight.data = torch.from_numpy(
            np.array(
                [
                    [
                        [
    ...
    def reconstruct_path(self):
        ...
        while not done:
            ...
            if count1 > 250:
                break
        return [coords.numpy() for coords in out_path], cost
